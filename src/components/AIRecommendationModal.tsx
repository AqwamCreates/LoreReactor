...
export function AIRecommendationModal({
    ...
    const injectPromptBlockImages = imageInjectionPriority.includes('promptBlock');
        ...
        <>
            ...
            <div className="modal-overlay" onClick={handleCloseForm}>
                <div className="modal-content editor-modal-content" onClick={e => e.stopPropagation()}>
                    <div className="modal-header"><h2>Get AI Recommendation</h2><div className="editor-modal-actions"><button type="button" className="editor-button editor-button-cancel" onClick={handleCloseForm} disabled={isGenerating || isUploadingImages}>Cancel</button></div></div>
                    <div className="modal-body editor-modal-body">
                        ...
                        <button type="button" className="editor-button editor-button-save entity-generate-button" onClick={handleGenerate} disabled={selectedEntities.length === 0 || isUploadingImages}>✨ Generate Recommendation</button>
                ...
                <div className="modal-overlay" onClick={handleCloseResult}>
                    <div className="modal-content editor-modal-content" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="modal-body editor-modal-body">
                            ...
                            {effectiveTab === 'Character' && hasChars && <div className="entity-field-list">
                                {parsedOutput!.characters!.map((char, i) => (
                                    <div key={char.id || i} style={{ marginBottom: '16px', padding: '8px', border: '1px solid var(--border)', borderRadius: '6px' }}>
                                        ...
                                        {onOpenCharacterEditor && <div style={{ marginTop: '8px', textAlign: 'center' }}><button type="button" className="editor-button editor-button-save" onClick={() => onOpenCharacterEditor(char, () => {})} style={editBtnStyle}>✏️ Open in Editor</button></div>}
                                    </div>
                                ))}
                            </div>}
                            ...
                            {effectiveTab === 'Location' && hasLocs && <div className="entity-field-list">
                                {parsedOutput!.locations!.map((loc, i) => (
                                    <div key={loc.id || i} style={{ marginBottom: '16px', padding: '8px', border: '1px solid var(--border)', borderRadius: '6px' }}>
                                        ...
                                        {onOpenLocationEditor && <div style={{ marginTop: '8px', textAlign: 'center' }}><button type="button" className="editor-button editor-button-save" onClick={() => onOpenLocationEditor(loc, () => {})} style={editBtnStyle}>✏️ Open in Editor</button></div>}
                                    </div>
                                ))}
                            </div>}
                            ...
                            {effectiveTab === 'AudioTrack' && hasTracks && <div className="entity-field-list">
                                {parsedOutput!.audioTracks!.map((track, i) => (
                                    <div key={track.id || i} style={{ marginBottom: '16px', padding: '8px', border: '1px solid var(--border)', borderRadius: '6px' }}>
                                        ...
                                        {onOpenAudioTrackEditor && <div style={{ marginTop: '8px', textAlign: 'center' }}><button type="button" className="editor-button editor-button-save" onClick={() => onOpenAudioTrackEditor(track, () => {})} style={editBtnStyle}>✏️ Open in Editor</button></div>}
                                    </div>
                                ))}
                            </div>}
                            ...
                            {effectiveTab === 'PromptBlock' && hasBlocks && <div className="entity-field-list">
                                {parsedOutput!.promptBlocks!.map((block, i) => (
                                    <div key={block.id || i} style={{ marginBottom: '16px', padding: '8px', border: '1px solid var(--border)', borderRadius: '6px' }}>
                                        ...
                                        {onOpenPromptBlockEditor && <div style={{ marginTop: '8px', textAlign: 'center' }}><button type="button" className="editor-button editor-button-save" onClick={() => onOpenPromptBlockEditor(block, () => {})} style={editBtnStyle}>✏️ Open in Editor</button></div>}
                                    </div>
                                ))}
                            </div>}
                            ...
                            <div className="entity-action-buttons">{isGenerating ? <button type="button" className="editor-button editor-button-cancel" onClick={handleStopGeneration} style={{ flex: 1 }}>⏹ Stop</button> : <><button type="button" className="editor-button editor-button-cancel" onClick={resetResult} disabled={isSaving} style={{ flex: 1 }}>Regenerate</button>{hasAnyParsed && <button type="button" className="editor-button editor-button-save" onClick={handleSave} disabled={isSaving} style={{ flex: 1 }}>{isSaving ? 'Saving...' : hasWorld ? '💾 Save World + All' : '💾 Save All'}</button>}</>}</div>
