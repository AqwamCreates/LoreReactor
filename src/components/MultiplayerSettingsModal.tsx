...
export function MultiplayerSettingsModal({
    ...
    const { save, loadForChat, multiplayerDataList, delete: deleteMultiplayerData } = useMultiplayerDataManager();
    ...
    // Auto-select the multiplayer data linked to current chat
    useEffect(() => {
        if (multiplayerData && !selectedMdId) {
            setSelectedMdId(multiplayerData.id);
        }
    }, [multiplayerData, selectedMdId]);

    // The currently viewed multiplayer data
    const activeMd = useMemo(() => {
        if (!selectedMdId) return null;
        if (multiplayerData?.id === selectedMdId) return multiplayerData;
        return multiplayerDataList.find(md => md.id === selectedMdId) ?? null;
    }, [selectedMdId, multiplayerData, multiplayerDataList]);

    // Sync local state when activeMd changes
    useEffect(() => {
        if (activeMd) {
            setPassword(activeMd.password || '');
            setAccountCharAssignments(activeMd.accountIdCharacterIds || {});
        } else {
            setPassword('');
    ...
    const isAdmin = useMemo(() => {
        if (!activeMd || !currentAccountId) return true;
        return activeMd.administratorAccountIds.includes(currentAccountId);
    }, [activeMd, currentAccountId]);
    ...
    }, [activeMd, password, accountCharAssignments, save, addToast]);

    const addAccountToList = useCallback((list: 'white' | 'black' | 'pending' | 'admin', accountId: string) => {
        if (!accountId.trim() || !activeMd) return;
    ...
    const removeAccountFromList = useCallback((list: 'white' | 'black' | 'pending' | 'admin', accountId: string) => {
        ...
        const field = fieldMap[list];
        const updated = { ...activeMd, [field]: activeMd[field].filter(id => id !== accountId), lastUpdatedTimestamp: Date.now() };
        save(updated);
    ...
    const approvePendingAccount = useCallback((accountId: string) => {
        ...
        const updated = {
            ...activeMd,
            pendingAccountIds: activeMd.pendingAccountIds.filter(id => id !== accountId),
    ...
    return (
        <>
            ...
            <ManagerModal
                ...
                actionLabel="Delete"
                activeSpecialActionId={selectedMdId ?? undefined}
            />

            {/* Right panel: Detail editor for selected session */}
            {selectedMdId && activeMd && (
                <div className="modal-overlay" onClick={() => setSelectedMdId(null)} style={{ zIndex: 9998 }}>
                    <div className="modal-content editor-modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: '700px' }}>
                        <div className="modal-header">
                            ...
                            <div className="editor-modal-actions">
                                ...
                                <button type="button" className="editor-button editor-button-cancel" onClick={() => setSelectedMdId(null)} style={{ fontSize: '0.75rem' }}>Back</button>
                            </div>
                        </div>
                                ...
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    ...
                                    <div className="editor-section" style={{ margin: 0 }}>
                                        ...
                                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                            {activeMd.interactionDataIds.map(chatId => (
                            ...
                            {activeTab === 'accounts' && (
                                <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                    ...
                                    {(activeMd.pendingAccountIds?.length ?? 0) > 0 && (
                                        <div className="editor-section" style={{ margin: 0, borderColor: 'rgba(234,179,8,0.3)' }}>
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                                                {activeMd.pendingAccountIds.map(accountId => (
                                    ...
                                    <div style={listSectionStyle}>
                                        ...
                                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                            {activeMd.whiteListedAccountIds?.map(id => (
                                    ...
                                    <div style={listSectionStyle}>
                                        ...
                                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                            {activeMd.blacklistedAccountIds?.map(id => (
                                    ...
                                    <div style={listSectionStyle}>
                                        ...
                                        <div style={{ display: 'flex', gap: '4px', flexWrap: 'wrap' }}>
                                            {activeMd.administratorAccountIds?.map(id => (
