...
export async function buildPromptAndStopPatterns(chatData: ChatData, character: Character, existingCharacterText: string, runtimePort?: number): Promise<BuildResult> {
    ...
    const callingOtherCharacterInstructions = `If the other character's name is provided, I must use their name instead of 'Character #' or 'Character # (Name)'. Otherwise I will use generic names or terms that ${characterParticipantTag} will likely use instead of 'Character #' or 'Character # (Name)'.`;
