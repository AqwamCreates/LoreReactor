...
const TOOL_INSTRUCTION_MAP: Record<tool, string> = {
    ...
    move: `${generalStartString}To move to an adjacent location, I write ${toolStartSring}move <location_id>${toolEndString}. The location must be connected via location bindings. Movement costs action stamina based on distance. The result will replace my tool call with confirmation or failure.${generalEndString}`,
    ...
    map: `${generalStartString}To check distances between locations, I write ${toolStartSring}map <location_id>${toolEndString} for distance from current location, or ${toolStartSring}map <loc1_id> to <loc2_id>${toolEndString} for distance between two specific locations. The distance and estimated travel time will replace my tool call.${generalEndString}`,
    audio: `${generalStartString}To play an audio track, I write ${toolStartSring}audio play <track_id>${toolEndString}. To stop a playing track, I write ${toolStartSring}audio stop <track_id>${toolEndString}. Only tracks marked as playable by participants can be controlled this way. Tool calls are replaced with confirmation.${generalEndString}`,
    clothing: `${generalStartString}To change what a character is wearing, I write ${toolStartSring}clothing <character_id> put on <clothing_id>${toolEndString} to wear an item, or ${toolStartSring}clothing <character_id> take off <clothing_id>${toolEndString} to remove it. The clothing item must exist in the character's wardrobe. Tool calls are replaced with confirmation of the clothing change.${generalEndString}`,
    ...
    invite: `${generalStartString}To bring an existing participant to my current location, I write ${toolStartSring}invite <character_id>${toolEndString}. The target must already be a participant in this session but not the protagonist. Tool calls are replaced with confirmation or failure.${generalEndString}`,
    kick: `${generalStartString}To remove a participant from the current location, I write ${toolStartSring}kick <character_id>${toolEndString}. This includes the protagonist. The target is moved to another location or removed from the scene. Tool calls are replaced with confirmation or failure.${generalEndString}`,
    teleport: `${generalStartString}To instantly move myself to any location regardless of adjacency, I write ${toolStartSring}teleport <location_id>${toolEndString}. This bypasses normal movement cost but consumes significant action stamina. Tool calls are replaced with confirmation or failure.${generalEndString}`,
    key: `${generalStartString}To lock or unlock a location, I write ${toolStartSring}key lock <location_id>${toolEndString} to prevent entry via binding triggers, or ${toolStartSring}key unlock <location_id>${toolEndString} to restore access. Tool calls are replaced with confirmation or failure.${generalEndString}`,
    summon: `${generalStartString}To add a non-participant character into this interaction session, I write ${toolStartSring}summon <character_id>${toolEndString}. The summoned character becomes a participant and can interact going forward. Tool calls are replaced with confirmation or failure.${generalEndString}`,
    ...
    inspect: `${generalStartString}To examine another character's visible state, I write ${toolStartSring}inspect <character_id>${toolEndString}. This reveals their name, current location, expression, worn clothing, and visible inventory items. Tool calls are replaced with the inspection results.${generalEndString}`,
    ...
    destroyer: `${generalStartString}To delete or destroy user-data entities, I write ${toolStartSring}destroyer <entity_type> <entity_id>${toolEndString}. This permanently removes the entity from the system. Use with extreme caution. Tool calls are replaced with confirmation or failure.${generalEndString}`,
};
